...
# load Patient subsample
load("DATA_full_DEATH.RData")
load("DATA_full_SURV.RData")
...
n.genes <- floor((length(VAR)/100)*5) # 10% # 5%
...
#topgenes  <- unique(c(topgenes1, topgenes2))


# Reduce data - VOOM
DATA_top_DEATH <- COUNTS_norm[topgenes, colnames(DATA_full_DEATH)]#DEATH_patients] 
DATA_top_SURV  <- COUNTS_norm[topgenes, colnames(DATA_full_SURV)]#SURV_patients]
...
res_DEATH  <- estimateTheta(R.input = rankMatrix_top, b = 0.1, num.boot = 100, solver = "gurobi",type ="restrictedQuadratic", bootstrap.type =  "classic.bootstrap", nCore=2)
...
##########################################
## ggplot
NAM = names(signal_sorted_DEATH[1:20])
NAM = sapply(strsplit(NAM,"|", fixed=TRUE), function(x){x[1]})
library(ggplot2)
#pdf("DEATH_ELBOW_quadratic.pdf")
plot.elbow <- ggplot(data = data.frame(),aes(x = 1:length(signal_sorted_DEATH[1:20]), y = signal_sorted_DEATH[1:20])) + geom_point() + geom_line() +
        geom_errorbar(aes(ymin=signal_sorted_DEATH[1:20]-signal_sorted_SE_DEATH[1:20], ymax=signal_sorted_DEATH[1:20] + signal_sorted_SE_DEATH[1:20]), width=.2,
                 position=position_dodge(0.05),colour="black") + 
        ylab("Signal estimate (theta)") +
        ylim(c(2,2.5)) +
        ggtitle("Top-20 genes of non-surviving patients") +
        theme(axis.text.x = element_text(size=14, angle=45)) + 
        scale_x_discrete(name ="Genes (ordered)", 
                    limits=NAM) 
	#theme_light()
plot.elbow #
#dev.off()
...
# Peform estimation via TopKSignal 
res_SURV  <- estimateTheta(R.input = rankMatrix_top, b = 0.1, num.boot = 100, solver = "gurobi",type ="restrictedQuadratic", bootstrap.type =  "classic.bootstrap", nCore=2)
...
##########################################
## ggplot
NAM = names(signal_sorted_SURV[1:20])
NAM = sapply(strsplit(NAM,"|", fixed=TRUE), function(x){x[1]})
library(ggplot2)
#pdf("SURVIVED_ELBOW_quadratic.pdf")
plot.elbow <- ggplot(data = data.frame(),aes(x = 1:length(signal_sorted_SURV[1:20]),y = signal_sorted_SURV[1:20])) + geom_point() + geom_line() +
        geom_errorbar(aes(ymin=signal_sorted_SURV[1:20]-signal_sorted_SE_SURV[1:20], ymax=signal_sorted_SURV[1:20] + signal_sorted_SE_SURV[1:20]), width=.2,
                 position=position_dodge(0.05),colour="black") + 
        #xlab("Genes (ordered)") +
        ylab("Signal estimate (theta)") +
        ylim(c(2,2.5)) +
        ggtitle("Top-20 genes of surviving patients")+ 
        theme(axis.text.x = element_text(size=14, angle=45))+
        scale_x_discrete(name ="Genes (ordered)", 
                    limits=NAM) 
	#theme_light()
